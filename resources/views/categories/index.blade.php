	...
	<div class="container">
		...
		<div class="row">
			<div class="col">
				...
				<!-- выводим пагинацию (с добавлением в строку запроса в браузере всех параметров) -->
...
<script>
	...
	$(document).ready(function() {
		$('.product_sorting_btn').click(function() {
			...
			// в переменную получим значение data-атрибута: order
			let orderBy = $(this).data('order')

			// в указанном классе после сортировки текст по умолчанию будет изменён на название сортировки
			$('.sorting_text').text($(this).find('span').text())

			// отправляем запрос методом: GET по указанному адресу
			$.ajax({
				...
				// в случае успешноо завершения ajax-запроса
				success: (data) => {
					// внесём изменения в адресную строку (что бы полученный результат сортировки можно было сохранить)
					let positionParameters = location.pathname.indexOf('?');
					let url = location.pathname.substring(positionParameters, location.pathname.length);
					let newURL = url + '?'; // http://127.0.0.1:8001/phones?
					newURL += "&page={{$_GET['page'] ?? 1}}" + 'orderBy=' + orderBy; // http://127.0.0.1:8001/phones?orderBy=name-z-a
					...
					// в указанный класс вставляем данные, полученные после сортировки в ProductController, которые вернул метод: showCategory()
					$('.product_grid').html(data)

					// применили плагин: isotope (чтобы не нарушились строй и размеры картинок отсортированных товаров)
					$('.product_grid').isotope('destroy')
					// подключили библиотеку: imagesLoaded в css-стилях
