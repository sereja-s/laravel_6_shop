...
<script>
	$(document).ready(function() {
		$('.cart_button').click(function(event) {
			// отменим стандартное поведение страницы при нажатии кнопки (чтобы не перекидывало вверх)
			event.preventDefault()
			addToCart()
		})
	})
	/* функция будет вызываться при клике на кнопку добавления в корзину */
	function addToCart() {
		let id = $('.details_name').data('id')
		// пердаём значение поля
		let qty = parseInt($('#quantity_input').val())

		// сделаем добавление в корзину (меняем значение счётчика)
			...
			<div class="col-lg-6">
				<div class="details_image">
					...
					<div class="details_image_thumbnails d-flex flex-row align-items-start justify-content-between">
						...
						<!-- если первая итерация цикла (1-я картинка) -->
						@if($loop->first)
